...
When ky follows redirects (the default behavior), cookies set on intermediate redirect responses (3xx) are not captured. This is a limitation of the [Fetch API redirect handling](https://fetch.spec.whatwg.org/#http-redirect-fetch:~:text=Return%20the%20result%20of%20running%20main%20fetch%20given%20fetchParams%20and%20recursive.) — when redirect mode is `"follow"`, intermediate responses are handled internally and not visible to hooks.
